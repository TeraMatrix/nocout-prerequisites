...
define(['underscore', 'jquery', 'chrome_lib'], function(_, $, lib) {
    ...
    function dismissed_alerts() {
        // provide an empty array if nothing is in lib.store
        return lib.store.get(STORE_KEY) || [];
    }
    ...
    function dismiss( alert ) {
        ...
        lib.store.set(STORE_KEY, dismissed);
        hide(alert);
        return alert_copy;
    }
    ...
    function undismiss( alert ) {
        ...
        lib.store.set(STORE_KEY, _.reject(dismissed_alerts(), prop_equal(alert, 'text')));
        show(alert);
        return alert;
    }
