...
define(['jquery', 'jsUri', 'analytics/events', 'analytics/attributes', 'analytics/adapters/omniture/attributes', 'chrome_lib', 's_code'], function ($, jsUri, events, attributes, om_attrs, lib) {
	...
	function get_cache (key) {
		if (typeof window.portal.cache !== 'undefined' && typeof window.portal.cache[key] !== 'undefined') {
			return window.portal.cache[key];
		}
		return undefined;
	}

	function set_cache (key, val) {
		if (typeof window.portal.cache === 'undefined') {
			window.portal.cache = {};
		}
		if (typeof val !== 'undefined') {
			window.portal.cache[key] = val;
		}
	}
	...
	event_definitions = {
		...
		LabsBegin : {
			values : {
				events: "event33",
				prop33: function () { return window.portal.analytics.attributes.get('ResourceType'); },
				eVar33: function () { return this.prop33(); }
			},
			call_tl: false,
			linkType: 'o',
		...
		LabsCompletion : {
			...
			should_call: function () {
				if (get_cache('labs_complete') === true) {
					return false;
				} else {
					set_cache('labs_complete', true);
					return true;
				}
			},
			linkType: 'o',
	...
	function add_event_value(s_obj, value) {
		if (typeof s_obj.events === 'undefined' || s_obj.events === "") {
			...
			if (typeof s_obj.events.indexOf !== 'undefined') {
				if (s_obj.events.indexOf(value) === -1) {
					s_obj.events = [s_obj.events, value].join(',');
				}
			}
		}
	}
