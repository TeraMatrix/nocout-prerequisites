...
define('chrome_lib', ['jquery', 'jsUri'], function ($, jsUri) {
	...
	lib = {
		...
		/**
		 * Store things in localStorage.  Because localStorage only accepts
		 * string values, lib.store will automatically serialize objects into
		 * JSON strings when you store them (set), and deserialize them back
		 * into objects when you retrieve them (get).
		 */
		store: {
